...
sub processPage {
	...
	foreach my $section ( split /^(?==)/m, $content ) {
		...
		my ($meanings) = $section =~ /\{\{znaczenia\}\}(.+?)\n\{\{/s;
		unless ( defined $meanings ) {
			$logger->warn("There is no 'znaczenia' section on $page->{title} for $language");
			next;
		}
		my @templates = $meanings =~ /\{\{([^{}\[\]]+)\}\}/g;
...
foreach my $index ( values %indexes ) {
	foreach my $language ( @{ $index->{languages} } ) {
		...
		my $content = "{{język linków|$language2}}{{TOCright}}";
