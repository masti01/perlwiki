...
sub getTemplate($$;$$) {
	...
	if ($notification) {
		return "{{subst:NPA $types{$type}/powiadomienie|url=$url|strona=$page|bot=$api->{login}|opiekun=msati}}\n";
	}
	else {
		return "{{NPA $types{$type}|url=$url|bot=$api->{login}|opiekun=masti}}";
	}
}
	...
	while ($attempt) {
		eval {
			...
			die "Unable to find the placeholder\n"
			  unless $content =~ s/(?<=<!-- mastiBot wstawia tutaj -->)/\n$report/;

			$api->edit(
				'title'          => $config{ReportTo},
				'starttimestamp' => $reportPage->{touched},
...
sub readConfig() {
	...
	push @exclude, qr{indietours\.rybnik\.pl}i;
	push @exclude, qr{www\.e-paryz\.eu}i;
	push @exclude, qr{hanower24\.ostroleka\.pl}i;
	push @exclude, qr{wafiso\.augustow\.pl}i;
	push @exclude, qr{malawi24\.eu}i;
	push @exclude, qr{amsterdam24x7\.malopolska\.pl}i;
	push @exclude, qr{gabon24x7\.eu}i;
